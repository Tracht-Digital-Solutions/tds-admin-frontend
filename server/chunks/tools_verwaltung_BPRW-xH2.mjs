...
import { D as apiFetch, k as toast, t as $$Layout, x as Spinner } from "./Layout_kCpQGkaf.mjs";
import { a as Presence, r as Collapse } from "./chunk-K3B6PMZC_OR-NiCIk.mjs";
...
function ToolsManage() {
	...
	const load = async () => {
		const res = await api$1("/admin/tools").catch(() => null);
		if (res === null) {
				...
				children: [/* @__PURE__ */ jsx("thead", { children: /* @__PURE__ */ jsxs("tr", { children: [
					...
					/* @__PURE__ */ jsx("td", {
						...
						children: /* @__PURE__ */ jsx("input", {
							...
							"aria-label": `${t.name}: Login erforderlich`
						})
					}),
					/* @__PURE__ */ jsx("td", {
						className: "text-center",
...
function ToolGuides() {
	...
	const load = async () => {
		const both = await Promise.all([api("/admin/tools"), api("/admin/tools/guides")]).catch(() => null);
		if (both === null) {
			setError("Tools konnten nicht geladen werden — die API ist nicht erreichbar.");
			setTools([]);
			return;
	...
	const reset = async () => {
		if (!toolId) return;
		setBusy(true);
		const res = await api(`/admin/tools/guides/${encodeURIComponent(toolId)}/${lang}`, { method: "DELETE" }).catch(() => null);
		setBusy(false);
		if (res === null) {
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			...
			/* @__PURE__ */ jsx(Presence, {
				view: toolId === "" ? "none" : `${toolId}-${lang}`,
				children: toolId === "" ? /* @__PURE__ */ jsx("div", {
					className: "tds-empty",
