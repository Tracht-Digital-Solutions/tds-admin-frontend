import { Z as __exportAll, d as renderTemplate, f as maybeRenderHead, i as renderComponent, m as addAttribute } from "./server_CzwR-3SR.mjs";
import { t as createComponent } from "./compiler_BXIK6MNx.mjs";
import { S as toast, _ as FormAlert, a as API_BASE, l as frontendFetch, p as FRONTEND_TARGET, t as $$Layout, x as Spinner } from "./Layout_b3SpCFRD.mjs";
...
var NS$8 = `${API_BASE}/admin/settings/mail`;
...
function MailSettings() {
	...
	const load = async () => {
		try {
			const [settingsRes, statusRes] = await Promise.all([frontendFetch(NS$8), frontendFetch(STATUS$1)]);
			if (!settingsRes.ok) {
	...
	const save = async () => {
		setBusy(true);
		try {
			const res = await frontendFetch(NS$8, {
				method: "PUT",
				headers: { "Content-Type": "application/json" },
...
var api$9 = apiFetch;
var NS$7 = "/admin/settings/billing";
/**
...
function BillingSettings() {
	...
	const [busy, setBusy] = useState(false);
	const load = async () => {
		const res = await api$9(NS$7).catch(() => null);
		if (res === null) {
...
var api$8 = apiFetch;
var NS$6 = "/admin/settings/lexware";
/**
...
function LexwareSettings() {
	...
	const [busy, setBusy] = useState(false);
	const load = async () => {
		const res = await api$8(NS$6).catch(() => null);
		if (res === null) {
			setStatus("Einstellungen konnten nicht geladen werden — die API ist nicht erreichbar.");
			setLoaded(true);
			return;
...
var api$6 = apiFetch;
var NS$5 = "/admin/settings/support-tickets";
...
function ImapSettings() {
	...
	const load = async () => {
		try {
			const [settingsRes, statusRes] = await Promise.all([api$6(NS$5), api$6(STATUS)]);
			if (!settingsRes.ok) {
				setError(settingsRes.status === 401 || settingsRes.status === 403 ? "Nur für Administratoren." : `Einstellungen konnten nicht geladen werden (HTTP ${settingsRes.status}).`);
				setLoaded(true);
				return;
...
var api$5 = apiFetch;
var NS$4 = "/admin/settings/live-chat-cta";
var FRONTENDS = [
	{
		key: "landingpage",
		label: "Landingpage"
...
function LiveChatSettings() {
	...
	const load = async () => {
		const res = await api$5(NS$4).catch(() => null);
		if (res === null) {
			setStatus("Einstellungen konnten nicht geladen werden — die API ist nicht erreichbar.");
			setLoaded(true);
			return;
...
var api$4 = apiFetch;
var NS$3 = "/admin/settings/website-cms";
/**
...
var api$2 = apiFetch;
var NS$2 = "/admin/settings/blog-cms";
/**
...
function ToolsSettings() {
	...
	const load = async () => {
		const res = await api(NS$1).catch(() => null);
		if (res === null) {
			setStatus("Einstellungen konnten nicht geladen werden — die API ist nicht erreichbar.");
			setLoaded(true);
			return;
